...
class Intersection:
    ...
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict,path_to_log):
        ...
        self.max_lane_num = dic_traffic_env_conf['max_lane_num']

        # generate all lanes
        direct_2_num = {'L':"0",'S':"1",'R':"2"}
        self.list_entering_lanes = [
            self.dic_entering_approach_to_edge["W"]+'_'+direct_2_num["S"],
            self.dic_entering_approach_to_edge["W"]+'_'+direct_2_num["R"],
            self.dic_entering_approach_to_edge["W"]+'_'+direct_2_num["L"],
            self.dic_entering_approach_to_edge["E"]+'_'+direct_2_num["S"],
            self.dic_entering_approach_to_edge["E"]+'_'+direct_2_num["R"],
            self.dic_entering_approach_to_edge["E"]+'_'+direct_2_num["L"],
            self.dic_entering_approach_to_edge["N"]+'_'+direct_2_num["S"],
            self.dic_entering_approach_to_edge["N"]+'_'+direct_2_num["R"],
            self.dic_entering_approach_to_edge["N"]+'_'+direct_2_num["L"],
            self.dic_entering_approach_to_edge["S"]+'_'+direct_2_num["S"],
            self.dic_entering_approach_to_edge["S"]+'_'+direct_2_num["R"],
            self.dic_entering_approach_to_edge["S"]+'_'+direct_2_num["L"]
            
        ]

        self.list_exiting_lanes=[
            self.dic_exiting_approach_to_edge["E"]+'_'+direct_2_num["S"],
            self.dic_exiting_approach_to_edge["S"]+'_'+direct_2_num["R"],
            self.dic_exiting_approach_to_edge["N"]+'_'+direct_2_num["L"],
            self.dic_exiting_approach_to_edge["W"]+'_'+direct_2_num["S"],
            self.dic_exiting_approach_to_edge["N"]+'_'+direct_2_num["R"],
            self.dic_exiting_approach_to_edge["S"]+'_'+direct_2_num["L"],
            self.dic_exiting_approach_to_edge["S"]+'_'+direct_2_num["S"],
            self.dic_exiting_approach_to_edge["W"]+'_'+direct_2_num["R"],
            self.dic_exiting_approach_to_edge["E"]+'_'+direct_2_num["L"],
            self.dic_exiting_approach_to_edge["N"]+'_'+direct_2_num["S"],
            self.dic_exiting_approach_to_edge["E"]+'_'+direct_2_num["R"],
            self.dic_exiting_approach_to_edge["W"]+'_'+direct_2_num["L"]
        ]
        ...
        for lane_id in self.list_entering_lanes:
            if lane_id in light_id_dict['adjacency_matrix_lane']:
                ...
                self._adjacency_row_lanes[lane_id] = [_get_top_k_lane([], self.dic_traffic_env_conf["TOP_K_ADJACENCY_LANE"]),_get_top_k_lane([], self.dic_traffic_env_conf["TOP_K_ADJACENCY_LANE"])]
    ...
    def _update_feature_map(self, simulator_state):
        ...
        dic_feature['reward'] = self.get_reward(dict())
    ...
    def _get_pressure(self):
        ...
        #6 N-->S
        #9 S-->N
        lane_v_num = self.eng.get_lane_waiting_vehicle_count()
        p_list = []
        dict_exit_lane = {}
        for i in [0,10,8]:
            dict_exit_lane[i] = [0,8,10] #E
        for i in [3,7,11]:
            dict_exit_lane[i] = [3,7,11] #W
        for i in [1,5,6]:
            dict_exit_lane[i] = [1,5,6] #S
        for i in [2,4,9]:
            dict_exit_lane[i] = [2,4,9] #N

        for i in [0,6,2,8]:
            p=0
            max_num = self.max_lane_num[i]
            enter_lane = self.list_entering_lanes[i]
            exit_lane_all = dict_exit_lane[i]
            s=0
            for e in exit_lane_all:
                s+=max_num - lane_v_num[self.list_exiting_lanes[e]]
            p+=min(lane_v_num[enter_lane],s)

            enter_lane = self.list_entering_lanes[i+3]
            exit_lane_all = dict_exit_lane[i+3]
            s=0
            for e in exit_lane_all:
                s+=max_num - lane_v_num[self.list_exiting_lanes[e]]
            p+=min(lane_v_num[enter_lane],s)
    ...
    def get_reward(self, dic_reward_info):
        reward = 0
        dic_lane_waiting_vehicle_count_current_step = self.dic_lane_waiting_vehicle_count_current_step
        for i in range(len(self.list_entering_lanes)):
            max_num = self.max_lane_num[i]
            lane_enter = self.list_entering_lanes[i]
            lane_exit = self.list_exiting_lanes[i]
            enter_count = dic_lane_waiting_vehicle_count_current_step[lane_enter]
            exit_count = dic_lane_waiting_vehicle_count_current_step[lane_exit]
            reward+=enter_count*(1-exit_count/max_num)
        return -reward
...
class AnonEnv:
    ...
    def _inner_step(self, action):
        ...
        #self.log_phase()
    def _get_pressure(self):
        return [inter._get_pressure() for inter in self.list_intersection]
