        ...
        for cnt_round in range(self.dic_exp_conf["NUM_ROUNDS"]):
            ...
            if multi_process:
                p = Process(target=model_test.test,
                            args=(self.dic_path, cnt_round, self.dic_exp_conf["RUN_COUNTS"], self.dic_traffic_env_conf, False))
                p.start()
                if self.dic_exp_conf["EARLY_STOP"]:
                    p.join()
            else:
                model_test.test(self.dic_path, cnt_round, self.dic_exp_conf["RUN_COUNTS"], self.dic_traffic_env_conf, if_gui=False)
