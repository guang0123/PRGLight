...
def test(dic_path, cnt_round, run_cnt, _dic_traffic_env_conf, if_gui):
    dic_traffic_env_conf = deepcopy(_dic_traffic_env_conf)
    model_round = "round_%d"%cnt_round
    dic_path = deepcopy(dic_path)
    ...
    try:
        ...
        for i in range(dic_traffic_env_conf['NUM_AGENTS']):
            agent = CoLightAgent(dic_agent_conf=dic_agent_conf,
                    dic_traffic_env_conf=dic_traffic_env_conf,
                    dic_path=dic_path,
                    cnt_round=1,  # useless
        ...
        max_greentime = 20
        pred_num = 20
        gnn = GNN(dic_path['PATH_TO_GNN_W'],dic_path['PATH_TO_GNN_V'])
        ...
        while not done and step_num < dic_exp_conf["RUN_COUNTS"]:
            ...
            if env.get_current_time()>pred_time[0]:
                ...
                max_greentime = min(50,max(20,max_greentime))
        ...
        import traceback
        error_dir = dic_path["PATH_TO_ERROR"]
